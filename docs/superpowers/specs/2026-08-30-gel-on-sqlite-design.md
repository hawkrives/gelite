...
## 9a. Baseline, as measured

The spec originally treated "does stock upstream Gel pass its own suite on this
fork?" as an open question gating Milestone 6. It has now been answered, and the
answer required clearing five stacked blockers first: the fork had no git tags
so nothing could build; `.github/workflows/*.yml` turned out to be generated
from `workflows.src` (since deleted, see §11); `astral-sh/setup-uv` publishes
no floating major tag;
`uv.lock` does not carry build dependencies, so `setup.py ci_helper` lost
Cython; and the test suite downloaded the `gel` CLI from `packages.edgedb.com`
at test time, which failed.

With those cleared, a shard reported:
...
All three failures were the dump/restore tests, failing on the CLI download.
`build`, `cargo-test` and `python-test-list` were green.

Two consequences for this design:

- **Milestone 6's gate is reachable.** The suite runs and substantially passes.
- **Scope shrank.** The dump/restore tests (10 modules, 37 fixtures, the
  `CLITestCaseMixin`/`DumpCompatTestCase`/`StableDumpTestCase` scaffolding) and
  the pgvector and AI extension tests were deleted rather than ported. PG-format
  dumps and pgvector both die with the Postgres backend, so neither was worth
  carrying through the port.

The `gel` CLI dependency is gone from the test harness entirely: `_admin_query`
now uses the in-tree client over the server's admin Unix socket.
...
Two additions from what CI established (Section 9a):

- **Rebuild branching coverage first**, before anything starts moving. The
  deleted `check_branching` created a schema or data branch, then verified
  schema fidelity by starting a migration to the original schema and asserting
  it reported `complete` immediately — a stronger check than comparing SDL text,
  whose rendering order is not stable. For data branches it re-ran the caller's
  own assertions against the branched database. Perhaps 40-60 lines on a plain
  `QueryTestCase`. This is the only thing that would catch a file-copy branch
  implementation copying a stale template, copying partially, or leaking rows
  into a schema-only branch.
- **Workflows are edited directly.** This was originally the opposite: the
  `.yml` files under `.github/workflows/` were rendered from
  `.github/workflows.src/` by `render.py`, driven by `.github/Makefile`, and
  carried no "do not edit" header, so direct edits were silently reverted.
  Both are now deleted. 741 lines of Jinja produced 796 lines of `tests.yml`
  — the 16 shards come from a native GHA `matrix:`, not from the templating,
  `tests.targets.yml` was empty, and nothing anywhere verified that the
  generated output still matched its source.
