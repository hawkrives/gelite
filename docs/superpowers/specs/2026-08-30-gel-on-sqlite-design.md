...
from `workflows.src`; `astral-sh/setup-uv` publishes no floating major tag;
...
- **Any workflow change goes through `.github/workflows.src`.** The `.yml` files
  under `.github/workflows/` are generated by `render.py` via `.github/Makefile`
  and carry no "do not edit" header. Editing them directly is silently reverted
  by the next `make -C .github`.
