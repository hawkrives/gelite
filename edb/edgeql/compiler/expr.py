...
def compile_GlobalExpr(
    ...
    if (
        ...
        if name not in ctx.env.query_globals_types:
            ...
            ctx.env.query_globals_types[name] = schemactx.derive_view(
                glob.get_target(ctx.env.schema), ctx=ctx
            )
...
def compile_TypeCast(
    ...
    if isinstance(expr.expr, (qlast.QueryParameter, qlast.FunctionParameter)):
        ...
        parameter_type = (
            ...
            if isinstance(expr.expr, qlast.QueryParameter)
            else irast.FunctionParameter
        )
...
def compile_Cursor(
    ...
    if cur_mod := ctx.modaliases.get(None):
        ...
        obj_type_names = [
            obj_type.get_name(ctx.env.schema).name for obj_type in obj_types
        ]
        obj_type_names.sort()
