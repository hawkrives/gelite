...
def assert_data_shape(
    ...
    def _assert_dict_shape(path, data, shape):
        if not isinstance(data, dict):
            fail(f'{message}: expected dict {_format_path(path)}')
    ...
    def _assert_set_shape(path, data, shape):
        if not isinstance(data, (list, set)):
            fail(f'{message}: expected list or set {_format_path(path)}')

        if not data and shape:
            fail(f'{message}: expected non-empty set {_format_path(path)}')
    ...
    def _assert_bag_shape(path, data, shape):
        ...
        if Ellipsis in shape:
            raise ValueError(f"{message}: can't use ellipsis in set/bag shape")

        data = list(data)

        if len(data) > len(shape):
            fail(f'{message}: too many elements in list {_format_path(path)}')
    ...
    def _assert_generic_shape(path, data, shape):
        if from_sql:
            if isinstance(shape, bag):
                ...
                if not math.isclose(
                    data, shape, rel_tol=rel_tol, abs_tol=abs_tol
                ):
                    fail(
                ...
                if data != shape:
                    fail(
                        f'{message}: {data!r} != {shape!r} {_format_path(path)}'
                    )
            elif isinstance(
                shape, (str, int, bytes, datetime.timedelta, decimal.Decimal)
            ):
                if data != shape:
                    fail(
                        f'{message}: {data!r} != {shape!r} {_format_path(path)}'
            ...
            if isinstance(shape, bag):
                ...
                if math.isnan(shape):
                    if not math.isnan(shape):
                        fail(f'NaN mismatch {_format_path(path)}')
